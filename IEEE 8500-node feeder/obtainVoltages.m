...
v3phaseRegs(1,:)=(inv(regulatorVoltageGains{1})*v3phase(find(strcmp(busNames,'regxfmr_HVMV_Sub_LSB')),:).').'; % three phases
v3phaseRegs(2,:)= (inv(regulatorVoltageGains{2})*v3phase(find(strcmp(busNames,'regxfmr_190-8593')),:).').'; % phase a 
v3phaseRegs(3,:)= (inv(regulatorVoltageGains{3})*v3phase(find(strcmp(busNames,'regxfmr_190-8581')),:).').'; % phase a and c
v3phaseRegs(4,:)= (inv(regulatorVoltageGains{4})*v3phase(find(strcmp(busNames,'regxfmr_190-7361')),:).').'; % three phase 
% else
