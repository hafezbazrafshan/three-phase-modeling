...
dsspath=[pwd,'/IEEE123openDSSdata'];
%% 1. DSS solution
 [Bus,Node1,Angle1,pu1,Node2,Angle2,pu2,Node3,Angle3,pu3] = importDSSvoltage([dsspath,'/ieee123_EXP_VOLTAGES.csv'])
...
for ii=1:length(busNamesWithRegs)
    ...
    busIdx=find(strcmp(lower(Bus),busName)); 
    if ~isempty(busIdx)
